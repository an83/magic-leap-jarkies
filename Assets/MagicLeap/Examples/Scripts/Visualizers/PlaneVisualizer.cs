...
namespace MagicLeap
{
    ...
    public class PlaneVisualizer : MonoBehaviour
    {
        ...
        private List<GameObject> _charcters = new List<GameObject>();
        ...
        private void DrawPlanes(MLWorldPlane[] planes)
        {
            ...
            for (int i = 0; i < planes.Length; ++i)
            {
                ...
                //if (isNew)
                //{
                //    AddCharctersOnPlane(planeVisual, planes[i]);
                //}
        ...
        private void AddCharctersOnPlane(GameObject planeVisual, MLWorldPlane plane)
        {
            if ((plane.Flags & (uint)OrientationFlags.Horizontal) == 0)
            {
                return;
            }
            ...
            {
                Debug.LogWarning("character prefab not defined");
                return;
            }

            if (_charcters.Count > 3) return;

            var character = Instantiate(CharacterPrefab, Vector3.zero, Quaternion.identity, planeVisual.transform);

            //var cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            character.transform.localScale = new Vector3(.25f, .25f, .25f);

            character.transform.SetParent(planeVisual.transform);
            character.transform.localPosition = Vector3.zero;

            _charcters.Add(character);
