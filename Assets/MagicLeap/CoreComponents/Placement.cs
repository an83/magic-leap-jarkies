...
namespace UnityEngine.XR.MagicLeap {
    ...
    public class Placement : MonoBehaviour {
        ...
        public float RotTilt = 0.0f;
        public float RotSpin = 0.0f;
        ...
        public Vector3 Position {
            ...
            private set;// { _position = value + new Vector3(Mathf.Acos(RotSpin), 0, -Mathf.Asin(RotSpin)); }
        }
        ...
        void Update() {
            ...
            //RotTilt += RaycastController.Singleton.gameObject.GetComponent<ControllerConnectionHandler>().ConnectedController.Touch1PosAndForce.y;
            RotSpin += ControllerManager.Singleton.gameObject.GetComponent<ControllerConnectionHandler>().ConnectedController.Touch1PosAndForce.x;
            RotTilt = Mathf.Clamp(RotTilt, -45, 45);
            if (RotSpin >= 360) { RotSpin -= 360; }
            if (RotSpin < 0) { RotSpin += 360; }

            Rotation = Quaternion.Euler(0, RotSpin, RotTilt);
        ...
        public void Confirm() {
            if (IsRunning && Fit == FitType.Fits) {
                IsRunning = false;
                _callback(Position + PositionOffset, Rotation);
            }
        }
        ...
        private void ValidateVolume() {
            if (_fitStatus == FitType.Fits) {
                ...
                Collider[] volumeCollisions = Physics.OverlapBox(centerPoint, collisionVolumeSize, Rotation, LayerMask.GetMask("Ignore Raycast"));

             /*   float PositionOffsetMag = -1.0f;
                while (volumeCollisions.Length > 0 && PositionOffset.magnitude > PositionOffsetMag) {
                    PositionOffsetMag = PositionOffset.magnitude;
                    float diag = collisionVolumeSize.magnitude;
                    Vector3 closestPoint = volumeCollisions[0].ClosestPoint(centerPoint);
                    Vector3 normal = (centerPoint - closestPoint).normalized;
                    PositionOffset = PositionOffset + normal*(diag - Physics.Raycast(centerPoint,);

                    print("moved by " + (centerPoint - volumeCollisions[0].ClosestPoint(centerPoint)));
                    centerPoint = Position + PositionOffset + (YAxis * (Scale.y * .5f));
                    collisionVolumeSize = Scale * (.5f - _maxUneven);
                    volumeCollisions = Physics.OverlapBox(centerPoint, collisionVolumeSize, Rotation, LayerMask.GetMask("Ignore Raycast"));
                }*/
